...
const useSiteNavigation = () => {
  ...
  const mapNavigationEntries = (entries: Navigation) => {
    // const getFormattedMessage = (label: string, key: string) => t.rich(label, context[key] || {});

    return Object.entries(entries).map(([key, { label, link, items, target }]): [string, MappedNavigationEntry] => [
      key,
      {
        target,
        label: label ? label : '',
